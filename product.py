...
class Product(tk.Frame):
	...
	def recreate_right_frame_3(self):
		self.detail_header.destroy()
	...
	def clicked_add_new_btn(self):
		...
		self.detail_add_header = tk.Frame(self.right_header, width=frame_w, height=frame_h, bg="green")
		self.detail_add_header.grid(row=0, column=0, sticky="nsew")

		self.virt_img = tk.PhotoImage(width=1, height=1)
		self.name_label = tk.Label(self.detail_add_header, text="Add New Product", font=("Arial", 30), width=frame_w, height=frame_h, image=self.virt_img, compound='c', bg="white")
		self.name_label.pack()
	...
	def clicked_add_product_btn(self):
		...
		self.last_current_product_index = len(self.settings.products) - 1
